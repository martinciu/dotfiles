...
THEMES = {
    ...
    "Everforest":"everforest","Kanagawa Wave":"kanagawa",
}
...
def usage_stats():
    """slug -> (rank, pct) parsed from `theme-set --stats --all`, so the page
    can list the themes actually used most, first. Returns {} when atuin / fish
    / the theme-set function aren't available (CI, fresh machine, sandbox) — the
    caller then falls back to definition order."""
    try:
        import subprocess
        text = subprocess.run(["fish","-c","theme-set --stats --all"],
                              capture_output=True, text=True, timeout=20).stdout
    except Exception:
        return {}
    known = set(THEMES.values()); stats = {}; rank = 0
    for line in text.splitlines():
        m = re.match(r'(\S+)\s+.+?\s+([\d.]+%)\s+\d+\s', line.strip())
        if not m: continue                       # skips header + footer lines
        slug, pct = m.group(1), m.group(2)
        if slug in known and slug not in stats:
            stats[slug] = (rank, pct); rank += 1
    return stats

if __name__ == "__main__":
    import sys
    # order themes by how long each has actually been active (theme-set --stats);
    # themes with no recorded usage keep definition order, after the used ones.
    stats = usage_stats()
    defrank = {slug:i for i,(_,slug) in enumerate(THEMES.items())}
    ordered = sorted(THEMES.items(),
                     key=lambda kv: stats[kv[1]][0] if kv[1] in stats else 100+defrank[kv[1]])
    ...
    for name, slug, cols, hexmap in themes_data:
        ...
        used = f" · {stats[slug][1]} used" if slug in stats else ""
        cards.append(f'''<section class="card" style="{style}">
      <header><h2>{html.escape(name)}</h2><span class="count">{len(cols)} colors · bg {bg}{used}</span></header>
      <div class="grid">{items}</div>
    </section>''')

    order_note = (" Themes are ordered by how long each has actually been the active theme "
                  "(<code>theme-set --stats</code>) — most-used first." if stats else "")
...
<style>
  ...
  *{{box-sizing:border-box;}}
  ...
  <p class="sub">Each distinct hex per theme, labelled with the variable / role name it's bound to in the
     ...
     <b>{len(grand)}</b> distinct colors total.{order_note}</p>
