...
assert_contains "$err" "LM Studio not reachable" "server-down reports LM Studio unreachable"
...
assert_contains "$err" "LM Studio not reachable" "leading-dash stdin still reaches friendly server-down error"
...
# 5. Live happy-path — only if LM Studio is reachable (keyless OpenAI-compat
#    endpoint; probe and slm share the same default URL).
